...
const TabsBottomIndicator = () => {
    const [state, setState] = useState<TabsAppProps>("puma");
    const spanRef = useRef<HTMLSpanElement>(null);

    useEffect(() => {
        const activeTrigger = document.getElementById(state) as HTMLElement | null;
        if (spanRef.current && activeTrigger) {
            spanRef.current.style.left =  activeTrigger.offsetLeft + 8 + "px";
            spanRef.current.style.width = activeTrigger.offsetWidth - 16 + "px";
        }
    }, [state]);

    return (
        <Tabs.Root className="w-[22rem] flex flex-col" defaultValue={state} onValueChange={(value) => setState(value as TabsAppProps)}>
            <Tabs.List className={tabsTheme.list + " w-max"} aria-label="stores">
                <span ref={spanRef} className={tabsTheme.indicator} aria-hidden="true"></span>
                <Tabs.Trigger className={tabsTheme.trigger} value="nike" id="nike">
                ...
                <Tabs.Trigger className={tabsTheme.trigger} value="adidas" id="adidas">
                ...
                <Tabs.Trigger className={tabsTheme.trigger} value="puma" id="puma">
            ...
            <Tabs.Content className="" value="nike">
                Nike
            </Tabs.Content>
